...
const countPublicFoldersByUser = `-- name: CountPublicFoldersByUser :one
SELECT COUNT(*) FROM folders WHERE user_id = $1 AND is_public = TRUE
`

func (q *Queries) CountPublicFoldersByUser(ctx context.Context, userID uuid.UUID) (int64, error) {
	row := q.db.QueryRowContext(ctx, countPublicFoldersByUser, userID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
const listPublicFoldersByUser = `-- name: ListPublicFoldersByUser :many
SELECT folder_id, user_id, name, description, created_at, updated_at, is_public FROM folders
...
type ListPublicFoldersByUserParams struct {
	UserID uuid.UUID `json:"user_id"`
	Limit  int32     `json:"limit"`
	Offset int32     `json:"offset"`
}

func (q *Queries) ListPublicFoldersByUser(ctx context.Context, arg ListPublicFoldersByUserParams) ([]Folder, error) {
	rows, err := q.db.QueryContext(ctx, listPublicFoldersByUser, arg.UserID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
