...
// UpdateCardPosition updates the position of a single card.
// Hand-written (not generated by sqlc).
const updateCardPosition = `UPDATE cards SET position = $1 WHERE card_id = $2 AND user_id = $3`
...
func (q *Queries) UpdateCardPosition(ctx context.Context, arg UpdateCardPositionParams) error {
	_, err := q.db.ExecContext(ctx, updateCardPosition, arg.Position, arg.CardID, arg.UserID)
	return err
}
