...
WHERE (NOT $1::boolean OR is_banned = TRUE)
`

func (q *Queries) CountUsers(ctx context.Context, filterBanned bool) (int64, error) {
	row := q.db.QueryRowContext(ctx, countUsers, filterBanned)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
type ListUsersParams struct {
	...
	FilterBanned bool  `json:"filter_banned"`
}

func (q *Queries) ListUsers(ctx context.Context, arg ListUsersParams) ([]User, error) {
	rows, err := q.db.QueryContext(ctx, listUsers, arg.Limit, arg.Offset, arg.FilterBanned)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
