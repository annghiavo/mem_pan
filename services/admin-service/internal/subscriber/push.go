...
func (h *PushHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.Unmarshal(body, &push); err != nil {
		log.Printf("[admin-pubsub] malformed envelope: %v", err)
	...
	if err != nil {
		log.Printf("[admin-pubsub] base64 decode error (msgID=%s): %v", push.Message.MessageID, err)
		w.WriteHeader(http.StatusNoContent)
		return
	}
	...
	if err := json.Unmarshal(raw, &env); err != nil {
		log.Printf("[admin-pubsub] malformed event envelope (msgID=%s): %v", push.Message.MessageID, err)
		w.WriteHeader(http.StatusNoContent)
		return
	}

	if err := h.handler.Dispatch(r.Context(), env.EventType, env.Data); err != nil {
		log.Printf("[admin-pubsub] dispatch %q error (msgID=%s): %v", env.EventType, push.Message.MessageID, err)
